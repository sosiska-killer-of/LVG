...
function Load()
	...
	--f_uidToPath = sm.terrainData.legacy_loadTerrainData( 1 )
	--g_cellData = sm.terrainData.legacy_loadTerrainData( 2 )
	
	--if f_uidToPath ~= nil and g_cellData ~= nil then
	--	return true
	--else
	--	return false
	--end
...
function GetTilePath( uid )
	if not uid:isNil() then
		if f_uidToPath[tostring(uid)] then
			if string.find( f_uidToPath[tostring(uid)], "$GAME_DATA/Terrain/Tiles/ClassicCreativeTiles/", 1, false ) == nil
			and string.find( f_uidToPath[tostring(uid)], "$GAME_DATA/Terrain/Tiles/CreativeTiles/", 1, false ) == nil then
				--print(string.gsub( f_uidToPath[tostring(uid)], "$GAME_DATA/Terrain/Tiles/", "$GAME_DATA/Terrain/Tiles/ClassicCreativeTiles/", 1 ))
				return string.gsub( f_uidToPath[tostring(uid)], "$GAME_DATA/Terrain/Tiles/", "$GAME_DATA/Terrain/Tiles/ClassicCreativeTiles/", 1 )
			end
			return f_uidToPath[tostring(uid)]
		else
			return GetPath( uid )
		end
	end
