...
function PortalGun.loadAnimations( self )
	...
	--portal
	self.portal = projectile_portal1
end
...
function PortalGun.client_onEquip( self, animate )
	...
	if self.tool:isLocal() then
		-- Sets PortalGun renderable, change this to change the mesh
...
function PortalGun.cl_onPrimaryUse( self, state )
	...
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		...
		if not sm.game.getEnableAmmoConsumption() or sm.container.canSpend( sm.localPlayer.getInventory(), obj_plantables_potato, 1 ) then
			...
			if owner then
				--portal
				sm.projectile.projectileAttack( self.portal, Damage, firePos, dir * fireMode.fireVelocity, owner, fakePosition, fakePositionSelf )
			end
...
--portal
function PortalGun.client_onReload(self)
	local invert = {}
	invert[projectile_portal1] = projectile_portal2
	invert[projectile_portal2] = projectile_portal1
	self.portal = invert[self.portal]
	if self.portal == projectile_portal1 then
		sm.gui.displayAlertText("#0088ffBlue portal")
	else
		sm.gui.displayAlertText("#ff8800Orange portal")
	end
